...
class SearchVisitor(FileVisitor):
    """
   Search files at and below start Dir for a string;
   subclass : redefine visitmatch,extension lists ,candidate as needed;
   subclasses can use testexts to specify file types to search (buy can also redefine candidate to use mimetypes for text content: see ahead)
    """
    skipexts=[]
    testexts= ['.txt','.py','.pyw','.html','.c','.h']
   #skipexts= ['.pyc','.gif','.jpg','.pyc','.o','.a','.exe']
...
if __name__=='__main__':
    ...
    def selftest(testmask):
        ...
        if testmask & dosearch:
            ...
            visitor.run(sys.argv[2])
        print('Found in %d files,visited %d '%(visitor.fcount,visitor.fcount))
