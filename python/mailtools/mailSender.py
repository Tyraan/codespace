...
class MailSender(MailTool):
    ...
    def __init__(self, smtpserver = None, tracesize = 256):
        self.smtpServerName = smtpserver or pp13.mailconfig.smtpservername
        self.tracesize = tracesize

    def sendMessage(self,
                    From, To, Subj, extrahdrs, bodytext,
                    attaches, saveMailSeparator = (('='*80 +'PY\n')),
    ...
    def addAttachments(self,mainmsg, bodytext, attaches,
        ...
        for (filename, fileencode) in zip(attaches, encodings):
            ...
            if contype is None or encodings is not None:
        ...
        mainmsg.preamble = 'A muti-part MIME format message \n'
        mainmsg.epilogue = ''

    def saveSentMessage(self, fullText, saveMailSeparator):
        try:
            sentfile = open(pp13.mailconfig.sentmailfile, 'a',encoding=pp13.mailconfig.fetchEncoding)
...
class MailSenderAuth(MailSender):
    ...
    def __init__(self,smtpserver=None,smtpuser = None):
        ...
        self.smtpUser = smtpuser or pp13.mailconfig.smtpuser
    ...
    def getpassword(self):
        if not self.smtpPassword:
            try:
                ...
                self.trace('localfile password' +repr(self.smtpPassword))

            except:
                MailSenderAuth.smtpPassword = self.askSmtpPassword()

    def askSmtpPassword(self):
        assert False,'subcalss must define this class'
